...
| 法規 | 在程式裡是什麼 |
| --- | --- |
| §15 第 1 款　執行法定職務必要範圍內蒐集處理 | 目的登記表。內建兩筆在 `lib/purposes.ts`；兌現機關在前端「登記台」掛上或下架，寫入 store。每個目的寫死允許的述詞集合與法源。 |
...
對話配對只走理解層（`lib/agent/intent.ts`），不再用正則猜你在說什麼。
...
沒有 key、逾時、回覆解析不了、標籤不在列舉裡——任何一種都當成沒聽懂，
回建議按鈕，**不會退回關鍵字比對**。演示腳本按鈕仍會打 `/api/chat`，所以
本機要測自由描述，請先接上 router。
