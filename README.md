...
每次辦一件事就簽一張授權，只涵蓋這一次、只有指定的那個機關能用、幾分鐘後就失效——一件事一張，分開裝，名字就是這麼來的。

要放行得同時滿足兩件事：你簽過名，而且機關能證明這張授權是發給它的。少一個都不行。就算你簽了，超出法定職務範圍的請求照樣擋下來。

誰、什麼時候、依哪一條、拿了哪幾項，全部留紀錄。可以撤銷；已經送出去的收不回來，畫面上也是這樣寫。

對申請人來說，差別是不用再自己跑一趟戶政、印一份謄本轉交，而且看得到誰在什麼時候要了什麼。不想給就不給，流程停在那裡，不會有東西被送出去。第二次辦同一件事，機關手上還在效期內的，不會再問你要一次。

對機關來說，差別是承辦人不必自己拿捏範圍——超出登記目的的請求在放行前就被擋掉，事後也查得到當時依的是哪一條。還有一件平常不太被講的好處：拿不到用不上的資料，也就不必保管它。少收一欄，就少一分外洩的風險和保存的責任。
...
打開之後上面有五頁，對應不同角色看到的東西。「授權」是上面這段對話。「金庫」列出你有哪些原始資料，哪幾欄被用來算出答案、哪幾欄從頭到尾沒動過、哪幾欄永遠不會被授權。「機關」是收件匣，站在社會局那一側看它收到了什麼——那頁有幾顆「試著越界」的按鈕，可以當場看它被擋下來以及擋下來的理由。「憑證」是同一份答案做成數位憑證的樣子。「登記台」是機關登記服務的地方。

## 機關這邊要做什麼

先看你是哪一種。要資料的那一方（例中的社會局），跟持有資料的那一方（戶政事務所），要做的事不一樣。

要資料的機關，把每個服務登記一次。「登記台」那一頁就是在做這件事，可以直接掛一筆新的上去試。

登記表要填的，就是[個資法 §8 第 1 項](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=I0050021)那六款——機關名稱、蒐集目的、資料類別、利用的期間與地區與對象與方式、當事人的權利、不提供會有什麼影響——加上這個目的依的是哪一條，以及授權最長能開多久。

少填一項就登記不上去。這不是刁難：這六款本來就是法律要求你告知當事人的，寫進登記表，民眾的同意畫面上才有東西可顯示。還要用白話寫一句「為什麼這幾項就夠、為什麼不要更多」，那句話會原封不動出現在他眼前。

掛上去之後那個目的就算數了：機關可以據它索取，各項檢查也會套用；下架一樣，改一列就好。但要提醒，這個示範裡「誰符合資格」的規則還是寫死的，只涵蓋內建那三個補助——新掛一個目的，它不會自己出現在民眾的清單上，得有人補上那條規則。

持有資料的機關要做的是另一件事：把每一項判斷做成一個能回答「是／否」或級距的介面。戶政不是交出一份謄本，而是回答「這個人設籍新北市嗎」。做過一次，任何登記過的服務都能問它，不必為每個補助各做一套。

登記表有一件事不讓你做：勾選還不存在的資料項。要一項新的，得持有那份資料的機關先把它做出來。這是刻意的——不然登記表就成了許願池，寫得出來卻沒人能回答。

## 同一份答案，也放得進國家的數位皮夾

上面那四個「是／否」不只能直接送給機關，也做成了憑證的樣子，放在「憑證」那一頁。

一份是 [SD-JWT](https://www.rfc-editor.org/rfc/rfc9901.html)，由戶政簽發。它的特性是出示的時候由你決定給幾題：卡片裡四個答案各自獨立，你可以只出示其中兩個，剩下兩個對方連摘要都還原不出來。攤開來看得到憑證本體裡只有雜湊值，沒有明文。

另一份送進[數位發展部的數位憑證皮夾沙盒](https://wallet.gov.tw/zh-tw)。發卡、產生 QR code、撤銷這幾支都接上了，也真的在沙盒上發過卡——過程中有四件事跟文件讀起來不一樣，包括認證用的標頭名稱、QR code 的取得方式，全部改成測試釘住而不是寫成註解。

這一層在授權那條路的旁邊，不在裡面。機關取資料走的還是原本那兩把鑰匙，一個字都沒改。

預設是關的，關著就不會發出任何網路請求。要打真的沙盒，`.env.local` 裡補上 `TWDIW_ENABLED=true` 和一把 API key；沒補的話畫面上會寫著停用中，以及缺哪一項。
