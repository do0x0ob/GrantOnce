...
2 選擇要辦哪一項 探索只列名單；選了才向該機關提出辦理申請
3 服務需求確認   機關依登記規格回覆本次必要資料，使用者決定要不要往下走
4 登記與法源檢查 確認之後才檢查：目的在登記表內、個資依據成立
5 最小化         通過檢查才鑄出授權匣，匣裡只有述詞
6 使用者簽署     閱讀告知事項後以 passkey 簽章
7 機關對機關交換 來源驗證簽章與持有證明，直接交付請求機關
8 服務處理       請求機關以述詞辦理案件
9 回覆與稽核     結果回覆使用者，每一步都留在稽核軌跡
```

第 2、3、4 步刻意分開。只是問問不會替任何服務留下紀錄；需求被確認之前，state 裡也不存在任何可簽署的匣——先問「這個人要不要辦」，才問「這個機關有沒有權力要」。反過來做，等於在替一個沒人開口要的服務先鑄好授權。
